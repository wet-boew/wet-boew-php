<?php
    // Validate and ensure specified theme is available.
    if (!in_array(__DIR__.'/'.$_SITE['wb_theme'], glob(__DIR__.'/theme-*', GLOB_ONLYDIR))) {
        $_SITE['wb_theme'] = preg_replace("/[^a-zA-Z0-9\-]+/", "", $_SITE['wb_theme']);
        if (isset($_COOKIE['theme'])) setcookie('theme', '', time()-3600, '/');
        die('The <strong>' . substr($_SITE['wb_theme'], 6) . '</strong> theme is not installed on this site.');
    }

    //
    // Add all of the default variables that may be null to prevent PHP warnings and errors for undefined values.
    //
    ...
    // Define the path to the wet php distribution folder for development the deploy folder is /wet-boew-php .
    ...
    // Include the theme specific configuration.
    include $_SERVER['DOCUMENT_ROOT'] . $_SITE['wb_php_dist_folder'] .'/config/'.$_SITE['wb_theme'].'/config.php';

    // If $_PAGE['lang1'] is set assume the user set the languages before including the config file. If so add only the required configuration files.
