...
<div id="wet-lang"><nav role="navigation"><h2><?php echo $_SITE['wet_langselect_'.$_PAGE['lang1']]; ?>
<?php
for( $i=2; isset($_PAGE['lang'.$i]); $i++ ) {
	if( isset($_SITE['wet_langselect_'.$_PAGE['lang'.$i]]) ) {
		echo " / ";
		echo "<span lang=\"".$_SITE['wb_meta_'.$_PAGE['lang'.$i]]."\">".$_SITE['wet_langselect_'.$_PAGE['lang'.$i]]."</span>";
	}
}
?>
</h2>
...
<div id="wet-wmms" class="span-2 margin-bottom-none"><div id="wet-wmms-in"><object data="<?php echo $_WMMS_IMG_; ?>.svg" role="img" tabindex="-1" aria-label="<?php echo $_WMMS_ALT_; ?>" type="image/svg+xml"><img src="<?php echo $_WMMS_FALL_IMG_; ?>.png" alt="<?php echo $_WMMS_ALT_; ?>" /></object></div></div>
<?php
//Print buttons for the other languages if they exist.
if( isset($_PAGE['lang3']) ) {
	echo '<div class="span-8 grid-12 margin-bottom-none">';
	echo '<ul id="wet-other-lang">' ."\n";
